...
For Alternative Situation 1, missing ozone values are correlated with a much higher Solar.R compared to existing ozone values. This means that it would be a bad idea to try to predict Solar.R using ozone. For Alternative Situation 2, we have a similar thing, and in addition, the missing ozone values are outside the range of Solar.R, so we also have the issue of extrapolation.
...
For wind and temp, it doesn't seem to matter if ozone is missing. For month, missing ozone values are on average one month behind exsisting ozone values. I would say that a linear regression model would be appropriate in this case.
