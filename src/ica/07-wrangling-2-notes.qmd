...
df_dates %>% 
  mutate(full_date_1 = str_c(month, "-", day, "-", year),
         full_date_2 = str_glue("{month}-{day}-{year}")) 
...
df <- tibble(
    word_id = 1:4,
    word = c("replace", "match", "pattern", "felicitous")
)

df %>%
  mutate(middle = str_sub(word, start = ceiling(str_length(word)/2), end = -ceiling(str_length(word)/2)))
...
```{r}
sum(str_count(posts$question_only, "(libido)"))
```

```{r}
str_view(posts$question_only, "(lgbtq)|(LGBTQ)")
```

```{r}
str_detect(posts$question_only[8], "(husband)")
```

```{r}
str_subset(posts$question_only, "(libido)")
```
